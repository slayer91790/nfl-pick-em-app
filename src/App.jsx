...
function App() {
  ...
  const funnySoundsRef = useRef(FUNNY_SOUND_FILES.map(file => new Audio(file))); // Audio Array
  const sanitizeEmail = (email) => email.replace(/\./g, '_');

  // --- 1. LOAD CONFIG & LOGIN LISTENER ---
  useEffect(() => {
    // Load Guest List & Config... (unchanged)
  }, []);
  ...
  const selectTeam = (gameId, teamAbbr, oddsString, targetPicksState, setTargetPicksState) => {
    ...
    if (oddsString && (oddsString.includes('+') || oddsString.includes('-'))) {
      ...
      if (match) {
        ...
        // --- FUNNY SOUND CRITERIA: Magnitude is 8 points or more ---
        if (magnitude >= 8) {
  ...
  // 👨‍💻 NEW: Admin Pick Submission Logic
  const submitAdminPicks = async () => {
    // ... (Submit logic for Admin target user)
